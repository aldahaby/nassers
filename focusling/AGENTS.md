...
## Focusling project rules

- Game rules live in `src/core/` as pure functions (state + `now` in, new state out). No React, I/O or `Date.now()` there.
- Tunable numbers live only in `src/config/`. Never hardcode prices, rates or thresholds in UI.
